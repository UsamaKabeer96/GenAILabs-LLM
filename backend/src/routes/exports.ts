...
export const exportRoutes = () => {
    ...
    router.get('/experiment/:id/csv', async (req: Request, res: Response) => {
        try {
            ...
            const csvRows = experiment.responses.map((response, index) => {
        ...
        try {
            ...
            const summary = {
                ...
                best_response: {
                    ...
                    text_preview: experiment.responses.find(r => r.id === comparison.best_response.response_id)?.text.substring(0, 200) + '...'
                },

                worst_response: {
                    parameters: comparison.worst_response.parameters,
                    score: comparison.worst_response.score,
                    text_preview: experiment.responses.find(r => r.id === comparison.worst_response.response_id)?.text.substring(0, 200) + '...'
                },

                // Average scores by parameter combination
                parameter_analysis: experiment.config.parameters.map((params, index) => ({
                    parameters: params,
                ...
                // Overall statistics
                overall_stats: {
                    avg_coherence: experiment.metrics.reduce((sum, m) => sum + m.coherence_score, 0) / experiment.metrics.length,
                    avg_completeness: experiment.metrics.reduce((sum, m) => sum + m.completeness_score, 0) / experiment.metrics.length,
                    avg_length: experiment.metrics.reduce((sum, m) => sum + m.length_score, 0) / experiment.metrics.length,
                    avg_structure: experiment.metrics.reduce((sum, m) => sum + m.structure_score, 0) / experiment.metrics.length,
                    avg_overall: experiment.metrics.reduce((sum, m) => sum + m.overall_score, 0) / experiment.metrics.length,
                    total_tokens: experiment.responses.reduce((sum, r) => sum + r.metadata.tokens_used, 0),
                    avg_response_time: experiment.responses.reduce((sum, r) => sum + r.metadata.response_time, 0) / experiment.responses.length
                }
            };
    ...
    router.get('/all/json', async (req: Request, res: Response) => {
        try {
            ...
            const experiments = await experimentController.getExperiments(limit, 0);
            ...
            const exportData = {
                ...
                experiments: experiments.map(experiment => ({
                    ...
                    avg_score: experiment.metrics.reduce((sum, m) => sum + m.overall_score, 0) / experiment.metrics.length
                }))
            };
